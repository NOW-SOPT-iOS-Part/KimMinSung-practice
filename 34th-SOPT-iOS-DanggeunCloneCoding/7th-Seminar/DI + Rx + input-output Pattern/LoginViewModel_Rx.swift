...
final class LoginViewModel_Rx: ViewModelType {
    ...
    struct Input {
        let idTextFieldTextObservable: Observable<String?>
        let passwordTextFieldTextObservable: Observable<String?>
        let loginButtonTapObservable: Observable<Void>
    }
    ...
    func transform(from input: Input, disposeBag: DisposeBag) -> Output {
        ...
        input.idTextFieldTextObservable
            .subscribe(onNext: { [weak self] id in self?.idText = id })
            .disposed(by: disposeBag)
        
        input.passwordTextFieldTextObservable
            .subscribe(onNext: { [weak self] password in self?.passwordText = password })
            .disposed(by: disposeBag)
        
        input.loginButtonTapObservable
